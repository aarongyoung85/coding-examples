/*
This is an example of a basic CRUD service for dealing with a user. For simplicity, organization is not prioritized here.
Usually, many of these components would be modularized or in separate libraries.
*/
...
// they're lower case because we don't want these to be exportable
...
func main() {
    ...
    // i include versions in the routes from the start so versioning won't be an issue moving forward
...
func (c *Controller) CreateUserHandler(rw http.ResponseWriter, req *http.Request) {
    // Golang's use of context is an area of much debate and i would gladly discuss it more
    // however, it's out of context for this coding example
	ctx := req.Context()
    lf := logrus.Fields{
        "handler": "CreateUser"
    }
	...
	if err != nil {
        ...
        // to me, it's super elegant and i love it
...
func (c *Controller) handleCreateUser(ctx context.Context, req *http.Request) (createUserResponse, error) {
    ...
    // explicit == readable and maintainable
